...
The exposition endeavours to build upon the foundations laid in \parencite{balanis:advanced-engineering-electromagnetics,griffiths:introduction-to-electrodynamics,zangwill:modern-electrodynamics}, incorporating personal insights and notational preferences to present a cohesive foundation for further chapters.
...
\begin{example}[TE waves in a rectangular waveguide]
    ...
    \begin{subequations}
        \label[pluralequation]{subeq:guided-waves-te-odes}
        \noindent\centering
        \begin{minipage}{0.45\textwidth}
            \begin{align}
    ...
    \Cref{eq:guided-waves-te-ode-x} is a simple second-order ordinary differential equation, with a general solution of the form
    \begin{align}
...
\section{Polarizer}
The design process of a waveguide polarizer is detailed in this section. An initial survey of existing literature, including relevant conference papers and research articles, is undertaken. Based on this survey, the performance characteristics of various design concepts are compared and contrasted. A solution to the design problem is then proposed, with a rationale provided for the selected approach. The chosen structure is subsequently implemented and optimized within CST Studio Suite for a specified target frequency band. The inherent trade-offs between key performance parameters are then discussed, with reference to established principles of guided wave propagation in rectangular waveguides. Finally, the simulated performance of the designed component is presented graphically, derived from full-wave simulations.

\subsection{Literature survey}
To achieve dual polarization, the inherent characteristics of symmetric waveguides, which support two orthogonal fundamental modes of propagation, are leveraged. This property forms the foundation for the design of the dual linear-to-circular polarizer presented in this work. This crucial characteristic arises from the combination of the waveguide's transverse coordinate symmetry and the fundamental orthogonality of distinct modes. The geometric symmetry of these waveguides (manifesting as mirror or rotational symmetry) allows for the existence of two fundamental modes with orthogonal polarization vectors in the transverse plane. A rectangular waveguide, as illustrated in \cref{example:te-waves-in-a-rectangular-waveguide}, exhibits this with itsTE$_{10}$ and TE$_{01}$ modes. Concurrently, \cref{remark:modal-decomposition-of-guided-waves} details a fundamental principle derived from Maxwell's equations which dictates that any two distinct modes within a waveguide are orthogonal. Mathematically, this orthogonality, expressed in \cref{eq:mode-orthogonality}, ensures that the power flow associated with the interaction of any two distinct modes is zero. Consequently, the transverse symmetry of the waveguide enables the existence of orthogonally polarized modes, while their inherent orthogonality guarantees their independent propagation without coupling or interference. This independent propagation is essential for polarization control, as it allows for the manipulation of the relative phase and amplitude of these orthogonal modes to achieve the desired polarization states, a capability directly leveraged in the design of the dual linear-to-circular polarizer detailed in this work. The desired polarization states of RHCP and LHCP were mathematically elaborated on in \cref{subsection:polarization}.

\paragraph{Continue with refining from here and onwards.}
This description of circular polarization establishes a clear objective for the design of a waveguide polarizer: the creation of a segment based on a symmetrical waveguide that introduces a one-quarter wavelength phase difference between its two mutually orthogonal modes along its length, while maintaining the magnitudes as equal as possible. During subsequent design stages, the latter criterion is transformed into a singular metric, known as the \emph{axial ratio} of the emitted wave, which describes the polarization purity based on its far-field properties. Several approaches--widely adopted in implementation--exist that we may refer to as standard methods for achieving these objectives. Notable mentions include the following:
\begin{itemize}
    \item \emph{Dielectric vane polarizers} feature a dielectric vane (a so-called quarter-wave plate) inserted into the segment of a symmetrical waveguide under an angle of $\pm 45\degree$ relative to the incident electrical field. The inserted dielectric introduces a difference in the propagation velocity of the two orthogonal modes along this segment. This is due to one of the modes being parallel to the vane, hence having its propagation velocity reduced, while the other is unaffected thanks to being perpendicular to the vane.
    \item \emph{Septum polarizers} have two rectangular ports that converge at a stepped septum and extend into a symmetrical waveguide. When fed through one of these ports, the septum polarizer converts approximately half of the energy to the opposite polarity, achieving circular polarization through a phase shift of one-quarter wavelength at the output port. Feeding the structure through the alternate port results in circular polarization in the opposite direction.
    \item \emph{Iris polarizers} utilize a symmetrical waveguide cross-section with ridges, also called corrugations or irises, typically positioned symmetrically on two opposing sides. These polarizers operate on a linearly polarized wave directed diagonally into the waveguide. In this configuration, the ridges appear to one of the waveguide's modes as inductive while to the other as capacitive. This differential treatment results in a phase delay of one-quarter wavelength at the output port.
\end{itemize}

While these established methods can be refined and adjusted to achieve favourable results in typical metrics for linear-to-circular polarizers, each also exhibits shortcomings in other respects. Dielectric vane polarizers encounter narrow bandwidth issues with significant power limitations due to inherent losses associated with the use of dielectric materials. In contrast, septum polarizers offer promising capabilities for a wide range of applications in terms of power handling and the ability to generate both right-hand and left-hand circular polarizations efficiently. However, their size and weight can pose constraints for many applications. Iris polarizers, while capable of converting linearly polarized input into both polarizations and operating at higher power levels, often exhibit \enquote{overmoding} in their structures and require precise mode-matching to maintain a good axial ratio across a broad frequency band. Moreover, designing irises involves complex mathematical analyses, adding to their operational challenges.

This thesis strives to design a novel approach aimed at achieving optimal polarization purity across a wide frequency band, utilizing a straightforward cross-sectional geometry. Beginning with the symmetrical structure of a square or circular waveguide, the polarization effect is achieved by inserting obstructive shapes into two opposing ends of the cross-section. Specifically, triangular prisms are introduced into the corners of the square waveguide, while protruding circular segments (maintaining rotational symmetry) are inserted opposite each other in the case of a circular waveguide. A linearly polarized wave launched into this structure is decomposed into two orthogonal modes, each of which propagates along electrical paths of different lengths, providing the required phase delay.

The cross-sections of such polarizers must be optimized to maximize phase difference per unit length while maintaining equal amplitudes as closely as possible. These objectives inherently conflict, necessitating a trade-off. From an application perspective, a balance must be struck between the length of the structure, which compensates for lower phase delay per unit length, and polarization purity, which is compromised by unequal mode magnitudes.

\newpage
\section{Feeding structure}
