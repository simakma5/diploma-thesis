...
\begin{example}[Monochromatic plane waves in free space]
    ...
    As will soon become evident, electromagnetic waves confined in waveguides are generally not transverse. The result we have just obtained is unique to lossless regions, i.e., regions with $\sigma = 0$, where the solution is not subject to boundary conditions.
\end{example}

\subsection{Guided waves}
Assumptions:
\begin{itemize}
    \item Materials: PEC with no surface sources, i.e., according to \cref{subeq:boundary-conditions}\\
    \begin{subequations}
        \noindent\centering
        \begin{minipage}{0.45\textwidth}
            \begin{align}
    ...
    \item Waves: Monochromatic waves that propagate down the tube, so $\vec E$ and $\vec B$ have the generic form (WLOS: $\hat{\vec k} \cdot \vec e_z = \hat{\vec k}$)
    \begin{subequations}
        \begin{align}
            \hat{\vec E}(\vec r,t) &= \hat{\vec E}_0(x,y)\exp\[\i\(\hat kz-\omega t\)\],
        \\
            \hat{\vec B}(\vec r,t) &= \hat{\vec B}_0(x,y)\exp\[\i\(\hat kz-\omega t\)\].
        \end{align}
    \end{subequations}
    \begin{subequations}
        \begin{align}
            \hat{\vec E}_0(x,y) &= E_x(x,y)\vec e_x + E_y(x,y)\vec e_y + E_z(x,y)\vec e_z,
        \\
            \hat{\vec B}_0(x,y) &= B_x(x,y)\vec e_x + B_y(x,y)\vec e_y + B_z(x,y)\vec e_z.
        \end{align}
    \end{subequations}

    \item Maxwell's equations: sourceless, i.e., \cref{subeq:maxwell-sourceless} which we will express in terms $\vec E$ and $\vec B$ in this sections due to the assumed linearity.
\end{itemize}

From \cref{eq:maxwell-sourceless-curl-e,eq:maxwell-sourceless-curl-h} for a linear medium\\
\begin{subequations}
    % \label[pluralequation]{subeq:maxwell-sourceless}
    \noindent\centering
    \begin{minipage}{0.45\textwidth}
        \begin{align}
            \partial_xE_y-\partial_yE_x &= \i\omega B_z\vphantom{\frac{\i\omega}{c^2}}
        \\
            \partial_yE_z-\i kE_y &= \i\omega B_x\vphantom{\frac{\i\omega}{c^2}}
        \\
            \i\hat kE_x - \partial_xE_z &= \i\omega B_y\vphantom{\frac{\i\omega}{c^2}}
        \end{align}
    \end{minipage}
    \hfill
    \begin{minipage}{0.45\textwidth}
        \begin{align}
            \partial_xB_y-\partial_yB_x &= -\frac{\i\omega}{c^2} E_z
        \\
            \partial_yB_z-\i\hat kB_y &= -\frac{\i\omega}{c^2} E_x
        \\
            \i\hat kB_x - \partial_xB_z &= -\frac{\i\omega}{c^2} E_y
        \end{align}
    \end{minipage}\bigskip
\end{subequations}\\
Equations b, c, e, and f can be solved for $E_x$, $E_y$, $B_x$, and $B_y$:
\begin{subequations}
    \begin{align}
        \label{eq:guided-wave-ex}
        E_x &= \frac{\i}{(\omega/c)^2-\hat k^2}\(\hat k\partial_xE_z+\omega\partial_yB_z\),
    \\
        \label{eq:guided-wave-ey}
        E_y &= \frac{\i}{(\omega/c)^2-\hat k^2}\(\hat k\partial_yE_z-\omega\partial_xB_z\),
    \\
        \label{eq:guided-wave-bx}
        B_x &= \frac{\i}{(\omega/c)^2-\hat k^2}\(\hat k\partial_xB_z-\frac{\omega}{c^2}\partial_yE_z\),
    \\
        \label{eq:guided-wave-by}
        B_y &= \frac{\i}{(\omega/c)^2-\hat k^2}\(\hat k\partial_yB_z+\frac{\omega}{c^2}\partial_xE_z\).
    \end{align}
\end{subequations}
Substituting all equations a, b, c, and d into \cref{eq:maxwell-sourceless-div-d,eq:maxwell-sourceless-div-b} yields uncoupled equations for $E_z$ and $B_z$:
\begin{subequations}
    \begin{align}
        \label{eq:guided-wave-ez}
        \(\partial^2_x+\partial^2_y+\(\frac{\omega}{c}\)^2-\hat k^2\)E_z &= 0,
    \\
        \label{eq:guided-wave-bz}
        \(\partial^2_x+\partial^2_y+\(\frac{\omega}{c}\)^2-\hat k^2\)B_z &= 0.
    \end{align}
\end{subequations}
...
\begin{align}
    YX''+XY''+\[\(\frac\omega c\)^2 - \hat k^2\]XY &= 0.
\end{align}
...
while
\begin{align}
    -k_x^2-k_y^2+\(\frac\omega c\)^2 - \hat k^2 &= 0.
\end{align}
The equations for $X$ and $Y$ are simple second order ODRs with the general solution
\begin{align}
...
\end{align}
That is the TE$_{mn}$ mode and the wave number is
\begin{align}
    \hat k &= \sqrt{\(\frac\omega c\)^2 - \(\frac{\pi m}a\)^2 - \(\frac{\pi n}b\)^2}.
\end{align}
Clearly, if
\begin{align}
    f < \frac c2\sqrt{\(\frac ma\)^2 + \(\frac nb\)^2} \equiv f_{mn},
\end{align}
where $f$ is frequency from $\omega = 2\pi f$, the wave number is imaginary. On such frequencies, the travelling wave becomes \emph{evanescent}, i.e., exponentially attenuated. For this reason, we talk about $f_{mn}$ as the \emph{cutoff frequency} for the TE$_{mn}$ mode in question.

\paragraph{TODO:} Derive the \emph{guide wavelength} and try to think if there is anything else worth talking about.
